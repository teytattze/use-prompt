...
import type {
  ListRecentInput,
  ListRecentOutput,
  ListRecentUseCasePort,
} from "@/module/prompt/port/list-recent-use-case.port";
...
export class ListRecentUseCase implements ListRecentUseCasePort {
  ...
  ): Promise<Result<ListRecentOutput, AppError>> {
    const result = await this.#promptRepository.findRecent(ctx, {
      filter: {
